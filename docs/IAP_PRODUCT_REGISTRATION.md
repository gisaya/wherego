...
| 공급가 | 450원 |
| 예상 판매가 | 490원(VAT 포함, 콘솔 자동 계산값 최종 확인) |
| 상품 이미지 | `assets/iap-product-3-credits.png` (1024 x 1024px PNG) |
| 할인 | 최초 등록 시 사용 안 함 |
| 노출 상태 | 샌드박스 테스트 때 ON, 운영 전환 전 최종 확인 |

콘솔에는 공급가 450원을 입력한다. 판매가는 VAT를 더해 자동 계산되므로 저장 전에 490원으로 표시되는지 최종 확인한다.
...
앱은 콘솔 상품 목록의 `displayName`, `description`, `displayAmount`, `iconUrl`을 그대로 표시하므로 결제 가격은 클라이언트에 하드코딩하지 않는다. 490원 문구는 상품 조회 실패 시 화면 레이아웃을 유지하기 위한 fallback일 뿐 결제 금액으로 사용되지 않는다.
