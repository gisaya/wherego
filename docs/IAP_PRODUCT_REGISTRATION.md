...
| --- | --- |
| 상품 유형 | 소모품 |
| 상품명 | AI 여행지 추천 3회 이용권 |
| 상품 설명 | 어디고 AI 여행지 추천을 3회 이용할 수 있어요. 이용권을 보유한 동안 배너광고가 노출되지 않고, 이용권 사용 시 결과 전 전면광고 없이 바로 추천받아요. 미사용 횟수는 만료 없이 보관돼요. |
| 공급가 | 450원 |
| 판매가 | 495원(VAT 포함, 콘솔 자동 계산) |
| 상품 이미지 | `assets/iap-product-3-credits.png` (1024 x 1024px PNG) |
| 할인 | 최초 등록 시 사용 안 함 |
| 노출 상태 | 샌드박스 테스트 때 ON, 운영 전환 전 최종 확인 |

콘솔에는 공급가 450원을 입력한다. 판매가는 VAT를 더한 495원으로 자동 계산된다. 공급가는 10원 단위만 입력할 수 있어 판매가 490원은 설정할 수 없다.
...
```text
WHEREGO_IAP_3_CREDIT_SKU=<콘솔에서 발급된 SKU>
```

기존 SKU를 그대로 전환하는 배포에서는 서버가 `WHEREGO_IAP_10_CREDIT_SKU`도 호환 입력으로 읽되 3회를 지급한다. 신규 설정과 이후 운영 문서에는 `WHEREGO_IAP_3_CREDIT_SKU`만 사용한다.

앱은 콘솔 상품 목록의 `displayName`, `description`, `displayAmount`, `iconUrl`을 그대로 표시하므로 결제 가격은 클라이언트에 하드코딩하지 않는다. 495원 문구는 상품 조회 실패 시 화면 레이아웃을 유지하기 위한 fallback일 뿐 결제 금액으로 사용되지 않는다.
...
2. 결제 성공 시 정확히 3회가 한 번만 지급되는지 확인
...
6. 환불 완료 주문의 3회가 회수되는지 확인
7. 앱 재설치 또는 다른 기기에서 토스 로그인 후 미사용 횟수가 복원되는지 확인
8. 유료 이용권 잔액이 있는 동안 질문·로딩·결과 배너가 노출되지 않는지 확인
9. 마지막 유료 횟수를 사용하는 추천 흐름에서도 결과 화면까지 배너가 노출되지 않는지 확인
