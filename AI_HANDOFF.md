...
- Codex `qc` 자동화는 6시간마다 최근 결과를 검사하고, 최신 코드에서도 재현되는 좁은 서버·질문 메타데이터 결함만 테스트 후 보완한다.
...
- 백엔드 Wherego 테스트 94개와 Python compileall 통과
- 장거리 실패 조합은 원후보 40개, 압축 11개, Gemini 후보 5개로 재현 성공했으며 모두 최소 거리 조건을 충족
- 운영 Render 커밋: `9c3df27c881a1dad7af268281e5660d476df039b`
...
- 후보 0건과 추천 집중도는 6시간 QC에서 계속 본다. 표본 10건 미만의 집중도는 결함으로 단정하지 않는다.
...
5. 6시간 QC와 보조 24시간 표본에서 후보 0건, 지역/거리 위반, 목적지 집중도를 재평가한다.
