...
- 첫 화면에서 전체 추천 잔여 횟수가 0이면 상태 문구를 `추천 횟수를 모두 사용했어요`로, 주 버튼을 `이용권 충전하기`로 전환해 기존 충전 화면으로 바로 이동한다. 일반·프로모션 진입에 모두 적용한다.
...
- 최신 AIT deploymentId: `019f6a5f-9522-7c05-9ab1-63bfa63fac2f`
...
3. 기본 2회, 광고 +1 최대 2회, 공유 +3, 구매 +10과 로그인 후 잔여 복원을 실기기에서 확인한다. 잔여 0회 첫 화면의 `이용권 충전하기` 버튼도 함께 확인한다.
