...
- 대표 이미지가 없으면 바다·자연·문화·야외활동 4종 공통 이미지를 컨셉에 맞게 사용하고 `관광공사 제공 이미지 없음` 또는 `AI 자체 추천 · 실제 사진 없음`을 표시한다.
...
- 결과 이미지가 없을 때 사용할 컨셉별 1280x720 공통 이미지 4종을 앱 자산에 추가하고 화면 카드와 1080x1350 저장 카드에 동일하게 연결했다.
...
- V2 선택 카드는 `따뜻하게 푹 쉬기`, `배 타고 둥실둥실` 같은 짧은 문구만 표시한다. 검색어·관광공사 분류·태그·제약은 서버 메타데이터로만 유지하고 보조문구는 렌더링하지 않는다.
...
- 백엔드 Wherego 테스트 124개와 Python compileall 통과
...
- 운영 질문 리소스 버전: `2026-07-20-v2`
- Android/iOS AIT 빌드 0 errors, 0 warnings 완료
- 최신 AIT deploymentId: `019f7f64-3e3f-78eb-b5c1-f4dd2f79932f`
- AIT 내부에 컨셉 이미지 4종, 운영 프로모션 코드, 50원과 라이브 광고 ID를 확인했고 프로모션 테스트 코드는 없다.
