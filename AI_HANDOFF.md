...
- The save output is PNG only. The hidden SVG card uses the system font instead of forcing Arial, and is kept transparently mounted in-bounds to reduce malformed image output risk. The current saved card uses a narrower centered card, a taller real KTO hero photo, a compact centered location row, and extra line capacity for recommendation reason/AI selection copy. Before submission, test on real Toss devices because `saveBase64Data` depends on native Toss app support.
